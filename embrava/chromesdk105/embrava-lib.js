...
async function SendBlyncUSB30ControlCommand(deviceInfo, byRedValue, byGreenValue, byBlueValue, byLightControl, byMusicControl_1, byMusicControl_2) {
    ...
    try {
		...
		if (deviceName === DEVICE_NAME_BLYNCLIGHT_MINI_41S) {
			// This is a WinUSB device
			...
			await device.sendReport(reportId, abyBlyncUsb30ReportBuffer);
...
async function RequestUSBDevice() {
	...
	// First request permission for Blyncusb Mini WinUSB version (Google SKU)
	
	const vendorId = 0x2c0d; // Blyncusb Mini WinUSB vid
    const productId = 0x0011;  // Blyncusb Mini WinUSB pid
	...
	// Next, request permission for Embrava Blyncusb HID devices
	try {
...
async function EnumerateDevices() {
    ...
    for (var device of devices) {
        do {
            ...
            if (device.vendorId == 0x2C0D && device.productId == 0x0011) {
